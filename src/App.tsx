...
import MidiInputControl from "./ui/MidiInput/MidiInputControl";
...
function App() {
  ...
  const onNoteOn = useCallback(
    (midi: number) => { scheduler?.noteAttack(midi); },
    [scheduler]
  );

  const onNoteOff = useCallback(
    (midi: number) => { scheduler?.noteRelease(midi); },
    [scheduler]
  );
  ...
  return (
    <div className="app">
      ...
      <MidiInputControl scheduler={scheduler} />
      <hr />
      <GridConfigPanel />
      <footer>
